...
<body>
	...
	if(is_array($_GET['sipIDs']))
  {
	...
	while ($siparisData = my_mysql_fetch_array($q)) {
		...
		$uq = my_mysql_query("select * from user where ID='".$siparisData['userID']."'");
		$u = my_mysql_fetch_array($uq);

		if(!$siparisData['name'])
		{
			$siparisData['name'] = $u['name'];
			$siparisData['lastname'] = $u['lastname'];
		}
		if(!$siparisData['address'])
		{
			$siparisData['address'] = $u['address'];
			$siparisData['semt'] = $u['semt'];
			$siparisData['city'] = $u['city'];
		}
		if(!$siparisData['ceptel'])
		{
			$siparisData['ceptel'] = $u['ceptel'];
		}
		?>
		<page size="A4">
			<div class="invoice">
						...
						<div class="col-md-6">
							<div class="bill-to">
								...
								//if ($siparisData['address2']) 
								{
									...
									<address>
										<?= $siparisData['firmaUnvani'] ?><br /><?= $siparisData['vergiDaire'] ?> / <?= $siparisData['vergiNo']?$siparisData['vergiNo']:$siparisData['tckNo'] ?>
										<br />
