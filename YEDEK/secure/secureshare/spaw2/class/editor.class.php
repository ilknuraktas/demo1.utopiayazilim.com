...
function dbekle($dbase, $icerik, $ozellikler, $id)
{
  ...
  global $tempInfo, $phpFileUploadErrors, $showSPError,$siteConfigArray,$addToFile;
  $dosyalar = array();
  ...
  for ($i = 0; $i < sizeof($icerik); $i++) {
    ...
    if (!$icerik[$i]['offline']) {
      ...
      if ($db == 'stok' && $dbase == 'urun' && hq("select count(*) from depo")) {
        $qx = my_mysql_query("select * from depo");
        while ($dx = my_mysql_fetch_array($qx)) {
          $lk = $dx['ID'];
          $q1arr[] = "`$db" . "_" . $lk . "`";
        }
  ...
  for ($i = 0; $i < sizeof($icerik); $i++) {
    ...
    if (!$icerik[$i]['offline']) {
      if (!$icerik[$i]['offline']) {
        ...
        if ($db == 'kdv' && $dbase == 'urun' && hq("select count(*) from ulkeler where kdv = 1")) {
          $qx = my_mysql_query("select * from ulkeler where kdv = 1");
          while ($dx = my_mysql_fetch_array($qx)) {
            $lk = $dx['ID'];
            $q2arr[] = "'" . $_POST[$db . '_' . $lk] . "'";
 ...
 // echo $query;
  ...
  foreach ($dosyalar as $dosyaID => $uploadto) {
    ...
    $tempInfo.=adminErrorv5("Guvensiz dosya tipi!");
      ...
      if (!move_uploaded_file($_FILES[$dosyaID]['tmp_name'], '../' . $uploadtoORG . '/' . $dosyaAdi)) {
        ...
        $tempInfo.=adminErrorv5("Dosya upload edilmedi! (".$phpFileUploadErrors[$_FILES[$dosyaID]["error"]].")");
      }
