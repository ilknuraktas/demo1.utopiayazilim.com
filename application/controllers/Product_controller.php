...
class Product_controller extends Home_Core_Controller
{
    ...
    public function start_selling_post()
    {
        ...
        if(!empty($_FILES['vergi']['name'])){



            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['vergi']['name'], PATHINFO_EXTENSION);
            $file_name1 = 'evrak_' . generate_unique_id() . "." . $ext;
            $file_name1_1 = 1;
            $config['file_name'] =$file_name1;
            $this->load->library('upload',$config);
            $date1=date('d/m/Y H:i:s');
            if($this->upload->do_upload('vergi')){
                $uploadData = $this->upload->data();
            }
        }else {
            $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name1 = $row['vergi'];
            $file_name1_1 = $row['vergi_access'];
            $date1=$row['vergi_date'];
        }
        if(!empty($_FILES['imza']['name'])){

            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['imza']['name'], PATHINFO_EXTENSION);
            $file_name2 = 'evrak_' . generate_unique_id() . "." . $ext;
            $config['file_name'] =$file_name2;
            $file_name1_2 = 1;
            $date2=date('d/m/Y H:i:s');
            $this->load->library('upload',$config);
            if($this->upload->do_upload('imza')){
                $uploadData = $this->upload->data();
            }
        }else {
        	            $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name2 = $row['imza'];
            $file_name1_2 = $row['imza_access'];
            $date2=$row['imza_date'];
        }
        if(!empty($_FILES['nufus']['name'])){

            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['nufus']['name'], PATHINFO_EXTENSION);
            $file_name3 = 'evrak_' . generate_unique_id() . "." . $ext;
            $config['file_name'] =$file_name3;
            $file_name1_3 = 1;
            $date3=date('d/m/Y H:i:s');
            $this->load->library('upload',$config);
            if($this->upload->do_upload('nufus')){
                $uploadData = $this->upload->data();
            }
        }else {
        	         $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name3 = $row['nufus'];
            $file_name1_3 =  $row['nufus_access'];
            $date3=$row['nufus_date'];
        }
        if(!empty($_FILES['magaza_sozlesmesi']['name'])){

            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['magaza_sozlesmesi']['name'], PATHINFO_EXTENSION);
            $file_name4 = 'evrak_' . generate_unique_id() . "." . $ext;
            $config['file_name'] =$file_name4;
            $file_name1_4 = 1;
            $date4=date('d/m/Y H:i:s');
            $this->load->library('upload',$config);
            if($this->upload->do_upload('magaza_sozlesmesi')){
                $uploadData = $this->upload->data();
            }
        }else {
        	         $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name4 = $row['magaza_sozlesmesi'];
            $file_name1_4 = $row['magaza_sozlesmesi_access'];
            $date4=$row['magaza_date'];
        }
        if(!empty($_FILES['satici_sozlesmesi']['name'])){

            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['satici_sozlesmesi']['name'], PATHINFO_EXTENSION);
            $file_name5 = 'evrak_' . generate_unique_id() . "." . $ext;
            $config['file_name'] =$file_name5;
            $file_name1_5 = 1;
            $date5=date('d/m/Y H:i:s');
            $this->load->library('upload',$config);
            if($this->upload->do_upload('satici_sozlesmesi')){
                $uploadData = $this->upload->data();
            }
        }else {
        	         $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name1_5 = $row['satici_sozlesmesi'];
            $file_name1_5 = $row['satici_sozlesmesi_access'];
            $date5=$row['satici_date'];
        }
        if(!empty($_FILES['gizlilik_sozlesmesi']['name'])){

            $config['upload_path'] = 'evrak/uploads/';
            $config['allowed_types'] = 'jpg|jpeg|png|gif|png|xlsx|pdf';
            $ext = pathinfo($_FILES['gizlilik_sozlesmesi']['name'], PATHINFO_EXTENSION);
            $file_name6 = 'evrak_' . generate_unique_id() . "." . $ext;
            $config['file_name'] =$file_name6;
            $file_name1_6 = 1;
            $date6=date('d/m/Y H:i:s');
            $this->load->library('upload',$config);
            if($this->upload->do_upload('gizlilik_sozlesmesi')){
                $uploadData = $this->upload->data();
            }
        }else {
        	         $query = $this->db->query("SELECT * FROM users WHERE id=".$user_id);
             $row = $query->row_array();
            $file_name6 = $row['gizlilik_sozlesmesi'];
            $file_name1_6 = $row['gizlilik_sozlesmesi'];
            $date6=$row['gizlilik_date'];
        }
        $data = array(
            ...
            'kep_adresi' => $this->input->post('kep_adresi', true),
            'vergi_no' => $this->input->post('vergi_no', true),
            'vergi_dairesi' => $this->input->post('vergi_dairesi', true),
            'ilce' => $this->input->post('ilce', true),
            'vergi' => $file_name1,
            'imza' => $file_name2,
            'nufus' => $file_name3,
            'magaza_sozlesmesi' => $file_name4,
            'satici_sozlesmesi' => $file_name5,
            'gizlilik_sozlesmesi' => $file_name6,
            'vergi_access' => $file_name1_1,
            'imza_access' => $file_name1_2,
            'nufus_access' => $file_name1_3,
            'magaza_sozlesmesi_access' => $file_name1_4,
            'satici_sozlesmesi_access' => $file_name1_5,
            'gizlilik_sozlesmesi_access' => $file_name1_6,
            'vergi_date' => $date1,
            'imza_date' => $date2,
            'nufus_date' => $date3,
            'magaza_date' => $date4,
        ...
        $sms_a = $sms_ayar->row();
        $this->auth_model->sms_send(05433423532,$sms_a->magaza_sistem);
        $this->auth_model->sms_send($this->input->post('phone_number', true),$sms_a->magaza_musteri);
    }
    /**
     * Add Product
     */
    ...
    public function toplu_duzenle($id)
    {
        //check auth
        if (!$this->auth_check) {
            redirect(lang_base_url());
        ...
        $this->load->view('earnings/partials/_header', $data);
        $this->load->view('product/edit_toplu_al');
        $this->load->view('earnings/partials/_footer');
    }
    ...
    public function toplu_urun_post()
    {
        //check auth
        if (!$this->auth_check) {
            redirect(lang_base_url());
        ...
        $product_id = $this->input->post('id', true);
        if ($this->product_model->product_toplu_model($product_id)) {
