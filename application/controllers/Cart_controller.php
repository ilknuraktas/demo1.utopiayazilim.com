        ...
        if ($mds_payment_type == 'promote') {
            ...
            if (!empty($order)) {
                ...
                if ($order->buyer_id == 0) {
                    ...
                    $sms_a = $sms_ayar->row();
                    $idler=$order_id;
                    $query = $this->db->query("SELECT * FROM order_products WHERE order_id=".$idler);
                    $row = $query->row();
                    $queryy = $this->db->query("SELECT * FROM users WHERE id=".$row->seller_id);
                    $roww = $queryy->row();
                    $this->auth_model->sms_send($roww->phone_number,$sms_a->siparis_satici);
                    $this->auth_model->sms_send($this->auth_user->phone_number,$sms_a->siparis_musteri);
