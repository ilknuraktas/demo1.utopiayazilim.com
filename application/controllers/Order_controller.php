    ...
    public function update_order_product_status_post()
    {
        ...
        if (!empty($order_product)) {
            if ($this->order_model->update_order_product_status($id)) {
                $query = $this->db->query("SELECT * FROM order_products WHERE id=" . $id);
                $row = $query->row();
                $queryy = $this->db->query("SELECT * FROM users WHERE id=" . $row->buyer_id);
                $roww = $queryy->row();
                $this->auth_model->sms_send($roww->phone_number, "Sipariş'iniz Gönderildi");
