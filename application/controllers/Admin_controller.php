...
class Admin_controller extends Admin_Core_Controller
{
    ...
    public function evrak_post()
    {
        $this->settings_model->evrak_post_time();
    ...
    public function evrak_list()
    {
        $data['title'] = trans("Evrak Listesi");

        $data['general_settings'] = $this->settings_model->get_general_settings();

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/settings/evrak_list', $data);
    ...
    */
    ...
    public function ad_spaces()
    {
        ...
        $data["array_ad_spaces"] = array(
			...
			"index_3" => trans("index_ad_space_3"),
    ...
    */
    ...
    {
        $data['title'] = trans("stories");
        $data["current_lang_id"] = $this->input->get("lang", true);

        if (empty($data["current_lang_id"])) {
            $data["current_lang_id"] = $this->general_settings->site_lang;
            redirect(admin_url() . "stories?lang=" . $data["current_lang_id"]);
        }

        $data['settings'] = $this->settings_model->get_settings($data["current_lang_id"]);
        $data['languages'] = $this->language_model->get_languages();
        $data['stories'] = $this->story_model->get_slider_items_all();
    ...
    public function sms_mesaj()
    {
        ...
        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/sms-mesaj', $data);
        $this->load->view('admin/includes/_footer');
    }

    public function add_story_item_post()
    {
        if ($this->story_model->add_item()) {
            $this->session->set_flashdata('success_form', trans("msg_story_added"));
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('error_form', trans("msg_error"));
            redirect($this->agent->referrer());
    ...
    public function gsm_update()
    {
        for ($i = 1; $i < 4; $i++){
        $data = array(
            'customer_id' => $this->input->post('customer_id'.$i, true),
            'username' => $this->input->post('username'.$i, true),
            'password' => $this->input->post('password'.$i, true),
            'phone' => $this->input->post('phone'.$i, true),
        );
        $this->db->where('id', $i);
        $this->db->update('gsm_api_users', $data);
    }

        return redirect($this->agent->referrer());
    }
    public function gsm_tercih()
    {
        for ($i = 1; $i < 4; $i++){
            $data = array(
                'status' => 0,
        ...
        $this->db->where('id', $this->input->post('status', true));
        $this->db->update('gsm_api_users', $data);
    ...
    public function sms_sablon()
    {
        ...
        $data = array(
            'mesaj_satici' => $this->input->post('mesaj_satici', true),
            'mesaj_musteri' => $this->input->post('mesaj_musteri', true),
            'siparis_satici' => $this->input->post('siparis_satici', true),
            'siparis_musteri' => $this->input->post('siparis_musteri', true),
            'magaza_sistem' => $this->input->post('magaza_sistem', true),
            'magaza_musteri' => $this->input->post('magaza_musteri', true)
        );
        $this->db->where('id', 1);
        $this->db->update('sms_sablon', $data);

        return redirect($this->agent->referrer());
    }


    public function delete_story_item_post()
    {
        $id = $this->input->post('id', true);
        if ($this->story_model->delete_slider_item($id)) {
            $this->session->set_flashdata('success', trans("msg_story_deleted"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
    }

    public function update_story_settings_post()
    {
        if ($this->story_model->update_slider_settings()) {
            $this->session->set_flashdata('success_form', trans("msg_updated"));
            $this->session->set_flashdata('msg_settings', 1);
        } else {
            $this->session->set_flashdata('error_form', trans("msg_error"));
    ...
    */
    ...
    public function bayi()
    {
        ...
        $data['users'] = $this->auth_model->get_bayi();


        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/users/bayi');
        $this->load->view('admin/includes/_footer');
    }
    ...
    public function magaza_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->magaza_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function vergi_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->vergi_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function nufus_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->nufus_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function imza_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->imza_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function satici_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->satici_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function gizlilik_aktif()
    {
        $user_id = $this->input->post('id', true);
        if ($this->auth_model->gizlilik_aktif($user_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function bayi_users()
    {
        $id = $this->input->post('id', true);
        if ($this->auth_model->bayi_users($id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
