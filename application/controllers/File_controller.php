			...
			foreach ($modesy_images as $modesy_image) {
				if ($modesy_image->file_id == $file_id) {
					echo '<img src="' . base_url() . "uploads/temp/" . $modesy_image->img_small . '" alt="">' .
						'<a href="javascript:void(0)" class="btn-primary btn-img-delete btn-delete-product-img-session" data-file-id="' . $modesy_image->file_id . '"><i class="mdi mdi-trash-can d-block font-size-16"></i></a>' .
						'<a href="javascript:void(0)" class="badge btn-success badge-secondary badge-is-image-main btn-set-image-main-session" data-file-id="' . $modesy_image->file_id . '">' . trans("main") . '</a>';
					break;
				}
			}
		}
	...
	public function get_uploaded_image()
	{
		...
		if (!empty($product_image)) {
			echo '<img src="' . get_product_image_url($product_image, 'image_small') . '" alt="">' .
				'<a href="javascript:void(0)" class="btn-img-delete btn-primary btn-delete-product-img" data-file-id="' . $product_image->id . '"><i class="mdi mdi-trash-can d-block font-size-16"></i></a>' .
				'<a href="javascript:void(0)" class="badge badge-secondary btn-success badge-is-image-main btn-set-image-main" data-image-id="' . $product_image->id . '" data-product-id="' . $product_image->product_id . '">' . trans("main") . '</a>';
		}
	}
