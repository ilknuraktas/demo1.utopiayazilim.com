...
if (!function_exists('get_store_rank')) {
    function get_store_rank($user_id){
        $ci=& get_instance();
        $ci->load->database(); 
        $sql ="SELECT COUNT(*) as total_sell FROM order_products WHERE seller_id = ".$user_id." and order_status = 'completed' "; 
...
if (!function_exists('get_store_rank_store')) {
    function get_store_rank_store($user_id){
        $ci=& get_instance();
        $ci->load->database(); 
        $sql ="SELECT COUNT(*) as total_sell FROM order_products WHERE seller_id = ".$user_id." and order_status = 'completed' "; 
        $query = $ci->db->query($sql);
        ...
        $count_item =  $row->total_sell;
        if($count_item  < 10){
            return ["<img src='/assets/ranks/acemi-satici.png' width='200'>"," Acemi Satıcı - Tecrübeli Satıcı Rütbesine Kalan :".(str_replace('-','',$count_item-30))." Sipariş"];
        }
        else if($count_item  < 30){
            return ["<img src='/assets/ranks/tecrubeli-satici.png' width='200'>"," Tecrübeli Satıcı - Hızlı Satıcı Rütbesine Kalan :".(str_replace('-','',$count_item-=75))." Sipariş"];
        }
        else if($count_item  < 75){
            return ["<img src='/assets/ranks/hizli-satici.png' width='200'>"," Hızlı Satıcı - Trend Satıcı Rütbesine Kalan :".(str_replace('-','',$count_item-150))." Sipariş"];
        }
        else if($count_item  < 150){
            return ["<img src='/assets/ranks/trend-satici.png' width='200'>"," Trend Satıcı - Tecrübeli Satıcı Rütbesine Kalan :".(str_replace('-','',$count_item-150))." Sipariş"];
        }
        else if($count_item > 150){
            return ["<img src='/assets/ranks/usta-satici.png' width='200'>"," Tecrübeli Satıcı"];
        }
        else{
            return $count_item;
        }
