...
if (!function_exists('get_best_sellers_product')) {
    function get_best_sellers_product()
    {
         $ci=& get_instance();
        $ci->load->database(); 
        $sql ="SELECT products.id, products.title, products.slug, products.product_type, products.listing_type, products.category_id, products.price, products.currency, products.discount_rate, products.user_id, products.is_promoted, products.rating, products.hit, products.is_free_product, products.promote_end_date, products.description, products.product_condition, products.created_at, (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY is_main DESC LIMIT 1) AS image, (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY is_main DESC LIMIT 1, 1) AS image_second, (SELECT COUNT(wishlist.id) FROM wishlist WHERE products.id = wishlist.product_id) AS wishlist_count FROM products LEFT JOIN images ON products.id = images.product_id WHERE products.visibility = 1 AND products.status = 1 GROUP BY product_id ORDER BY hit DESC LIMIT 0,12"; 
...
if (!function_exists('get_home_categories')) {
    function get_home_categories($category_id){
         $ci =& get_instance();
         return  $ci->product_model->get_paginated_filtered_products($category_id,0,6);
    }
}
if (!function_exists('get_trend_products')) {
    function get_trend_products()
    {
         $ci=& get_instance();
        $ci->load->database(); 
        $sql ="SELECT products.id, products.title, products.slug, products.product_type, products.listing_type, products.category_id, products.price, products.currency, products.discount_rate, products.user_id, products.is_promoted,products.is_trend, products.rating, products.hit, products.is_free_product, products.promote_end_date, products.description, products.product_condition, products.created_at, (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY is_main DESC LIMIT 1) AS image, (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY is_main DESC LIMIT 1, 1) AS image_second, (SELECT COUNT(wishlist.id) FROM wishlist WHERE products.id = wishlist.product_id) AS wishlist_count FROM products LEFT JOIN images ON products.id = images.product_id WHERE products.visibility = 1 AND products.status = 1 AND products.is_trend = 1 GROUP BY product_id ORDER BY hit DESC LIMIT 0,12"; 
        $query = $ci->db->query($sql);
        return $query->result();
    }
}
