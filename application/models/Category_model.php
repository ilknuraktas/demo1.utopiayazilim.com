...
class Category_model extends CI_Model
{
    ...
    public function input_values()
    {
        $data = array(
            ...
            'home_show' => $this->input->post('home_show',true),
    ...
    public function get_paginated_categories($per_page, $offset)
    {
        ...
        if (!empty($q)) {
            ...
            $query = $this->db->query($sql, array($like, clean_number($offset), clean_number($per_page)));
        } else {
    ...
    {
        $sql = $this->build_query() . "WHERE home_show = 1 AND categories.visibility = 1";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function get_all_parent_home_categories()
    {
        $sql = $this->build_query() . "WHERE categories.parent_id = 0 AND home_show = 1 ORDER BY category_order";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function get_subcategories_by_parent_id_except_one($parent_id, $except_id)
    {
        ...
        $query = $this->db->query($sql, array(clean_number($except_id), clean_number($parent_id)));
        return $query->result();
    }
