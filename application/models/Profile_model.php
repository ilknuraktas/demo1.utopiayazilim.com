...
class Profile_model extends CI_Model
{
    ...
    public function set_mng_api_account($user_id)
    {
        $user_id = $this->auth_user->id;
        $data = array(
            'mng_user' => $this->input->post('mng_user', true),
            'mng_pass' => $this->input->post('mng_pass', true)
        );

        $this->db->where('id', $user_id);
        $this->db->update('users', $data);
        
        $this->db->update('product_options', ['status' => false]);
        
        $this->db->where('common_id','6286c5284c9861-06178257-17104784');
        return $this->db->update('product_options', ['status' => $this->input->post('status', true)]);
    }
    
    public function set_yurtici_api_account($user_id)
    {
        $user_id = $this->auth_user->id;
        $data = array(
            'yurtici_user' => $this->input->post('yurtici_user', true),
            'yurtici_pw' => $this->input->post('yurtici_pw', true)
        );
        ...
        $this->db->where('common_id','6273fd4e13e618-41460273-99112736');
        return $this->db->update('product_options', ['status' => $this->input->post('status', true)]);
    }
    

    public function set_ptt_api_account($user_id)
    {
        $user_id = $this->auth_user->id;
        $data = array(
            'ptt_id' => $this->input->post('ptt_id', true),
            'ptt_pass' => $this->input->post('ptt_pass', true)
        );
        ...
        $this->db->where('common_id','6275d53380a934-59502959-22116944');
        return $this->db->update('product_options', ['status' => $this->input->post('status', true)]);
    }
    ...
    public function edit_user($id)
    {
        ...
        if (!empty($user)) {
            $data = array(
                ...
                'iskonto' => $this->input->post('iskonto', true),
