...
class Product_model extends Core_Model
{
    ...
    public function add_product()
    {
        $data = array(
            ...
            'GTIN_code' => $this->input->post('GTIN_code', true),
            'price' => 0,
            'currency' => "",
            'discount_rate' => 0,
            'vat_rate' => 0,
            ...
            'hit' => 0,
            'demo_url' => "",
            'external_link' =>  $this->input->post('external_link', false),
            'files_included' => "",
            'stock' => 1,
            'shipping_time' => "",
            'shipping_cost_type' => "",
    ...
    public function product_toplu_model($id)
    {
        $id = clean_number($id);
        $data = array(
            'alinacak_miktar' => $this->input->post('alinacak', true),
            'odenecek_miktar' => $this->input->post('odenecek', true)
        );
        $this->db->where('id', $id);
    ...
    public function edit_product($product)
    {
        $data = array(
            ...
            'GTIN_code' => $this->input->post('GTIN_code', true),
                        ...
                        'is_trend' => $this->input->post('is_trend', true),
    ...
    public function get_user_all_products($user_id)
    {
        $sql = $this->query_string() . "AND users.id = ? ORDER BY products.created_at DESC";
        $query = $this->db->query($sql, array(clean_number($user_id)));
        return $query->result();
    }
