(function ($) {
  ...
  Utopia.behaviors.fontawesome_icon_init = {
    attach: function (context, settings) {
      ...
      var container = $('.brk-fa-icons-container');
      var shown = 50;

      $('.brk-fa-icons-more button').on('click', function(){
        var next = shown + 50 < icons.length ? shown + 50 : icons.length;
        
        if (next !== shown) {
          for (shown; i < next; i++){
            container.append('<div class="brk-fa-icon-container"> <div class="brk-fa-icon-container__icon">  <i class="'+icons[i]+'"></i>  </div> <p class="brk-fa-icon-container__name">'+icons[i]+'</p> </div>');
          }
        }

        shown = shown + 50 < icons.length ? shown + 50 : icons.length;
      });
